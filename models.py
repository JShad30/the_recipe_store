...
class Recipe(db.Model):
    ...
    recipe_difficulty = db.Column(db.String(10), nullable=False)
    recipe_prep_time = db.Column(db.String(20), nullable=False)
    recipe_cook_time = db.Column(db.String(20), nullable=False)
    ...
    instruction = db.relationship('Instruction', backref="recipe", lazy=True)
    
    def __repr__(self):
        return 'Recipe({0}, {1}, {2}, {3}, {4}, {5})'.format(self.recipe_name, self.recipe_intro, self.recipe_description, self.recipe_image_file, self.recipe_added, self.recipe_score)
